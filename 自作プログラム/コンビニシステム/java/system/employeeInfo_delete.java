 ...
 * @author guest1Day
 */
public class employeeInfo_delete extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            db_st = db_con.prepareStatement("UPDATE employee SET status=-1 WHERE ID=?");
            db_st.setInt(1,employee.getID());
            int row = db_st.executeUpdate();

            db_st.close();
            db_con.close();
            hs.removeAttribute("delete");
            request.setAttribute("deletedEmployee",employee);
